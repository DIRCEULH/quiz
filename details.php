...
{
	$sql = "SELECT *FROM answer_user where user = '".$_SESSION['user']."' ORDER BY id DESC LIMIT 1";
...
if (!empty($_SESSION['user']))
{
	$sql = "SELECT  answers_id FROM answer_user_id where user = '".$_SESSION['user']."'";
	$stm = $connection->prepare($sql);
	$stm->execute();
...
if (!empty($_SESSION['user']))
{
	$sql = " SELECT *FROM answer_user_id e WHERE e.answers_id  NOT IN (SELECT answer_id FROM questions  ) and e.user ='".$_SESSION['user']."'";  
	$stm = $connection->prepare($sql);
	$stm->execute();
